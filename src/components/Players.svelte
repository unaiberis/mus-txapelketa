    ...
    try {
      ...
      if (wrapperEl) {
        try {
          ...
          if (ok) {
            // add mesa numbers to first-round matches (wait for async insertion)
            await addMesaBadges(bracketryData);
  ...
  async function renderManagerViewer(data?: any) {
    ...
    try {
      ...
      if (wrapperEl) {
        ...
        if (ok) {
          // add mesa numbers to first-round matches
          await addMesaBadges(bracketData);
